(function () {
	...
	function renderControl(field) {
		...
		if (field.control === 'numberAffix') {
			return renderNumberAffix(field);
		}

		if (field.control === 'uneditable') {
			return renderUneditable(field);
		}
	...
	function renderGridRowControls(actions, rowIndex) {
		...
		return `<td class="saGridRowControls"><ul>${buttons}</ul></td>`;
	}

	function renderGridHeader(column, index) {
		const classes = [column.align === 'right' ? 'right' : '', column.align === 'center' ? 'center' : ''].filter(Boolean).join(' ');
		...
		const inner = column.sortable === false
			...
			: `<a class="saGridHeadingInner sort${column.sorted ? ' saSorted' : ''}" tabindex="0" data-col-index="${index}"><span class="saGridHeadingLabel">${escapeHtml(column.label)}</span>${column.sorted ? '<i class="saIcon fas fa-caret-up" aria-hidden="true"></i>' : ''}</a>`;

		return `
			<th${classes ? ` class="${classes}"` : ''}${style}>
				${inner}
				<img src="img/transparent.png" class="draghandle" draggable="true" aria-hidden="true" data-col-index="${index}" style="height: 30px;">
			</th>`;
	}

	function renderGridCellValue(value) {
		if (value && typeof value === 'object') {
			const icon = value.icon ? `<span class="systemicon">${iconHtml(value.icon, value.iconClass || '')}</span>` : '';
	...
	function renderGridCell(column, row) {
		...
		return `
			<td${classes ? ` class="${classes}"` : ''}${style}>
				<div class="saGridCell">${renderGridCellValue(value)}</div>
			</td>`;
	}
		...
		return `
			<div class="saGridTop">
				<div class="saGridTopInner">
					<div class="saGridHitCounter">${escapeHtml(component.hitCounter || `${(component.rows || []).length} of ${(component.rows || []).length} hits`)}</div>
					${renderGridPagination(component)}
					<div class="saButtons">
						<label class="saInputTextWrapper saLabeled saGroupingGrid">
							<span class="saLabeledLabel saGroupingGridLabel">Group by</span>
							<div class="saTrailingIconsWrapper"><i class="saIcon far fa-angle-down"></i></div>
							<select class="saInputText saDropdown saGridGroupingDropdown">
								${groupOptions.map((option, index) => `<option value="${index}">${escapeHtml(option)}</option>`).join('')}
							</select>
						</label>
						<button class="saGridButton" type="button" data-tooltip="Show, arrange and sort" aria-label="Show, arrange and sort"><span class="saIcon far fa-sliders" aria-hidden="true"></span></button>
	...
	function renderResultGrid(component) {
		...
		return `
			<div class="maincolbody grid saMockLargeScreenOnly">
					...
					<table class="saGrid">
						...
						<thead class="saGridHead stickyheader">
							<tr class="saGridHeadingRow" tabindex="0" aria-label="Use arrow keys to adjust column width. Press space or enter to sort.">
								<th class="saGridRowControls saGridSpecialHeader" colspan="1"><ul></ul></th>
								${columns.map(renderGridHeader).join('')}
							</tr>
						</thead>
						<tbody>
							${rows.map((row, index) => `
								<tr class="saGridRow hascontextmenu${index > 0 && index % 5 === 0 ? ' saGridRowEvery5th' : ''}" data-sa-row-id="${index}">
									${renderGridRowControls(row.actions || component.rowActions, index)}
	...
	function renderComponent(component) {
		if (component.type === 'MenuGroups') {
			return renderMenuGroups(component);
		}

		if (component.type === 'RadioCards') {
			return renderRadioCards(component);
		}

		if (component.type === 'NewEdit') {
			return renderNewEdit(component);
		}

		if (component.type === 'InfoBoxes') {
			return renderInfoBoxes(component);
		}

		if (component.type === 'ResultGrid') {
			return renderResultGrid(component);
		}

		if (component.type === 'Multipart') {
			return renderMultipart(component);
		}

		if (component.type === 'CalendarWeekdays') {
			return renderCalendarWeekdays(component);
		}

		if (component.type === 'DetailView') {
			return renderDetailView(component);
		}
