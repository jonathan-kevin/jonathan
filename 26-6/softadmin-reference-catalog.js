(function () {
	const catalog = {
		components: {
			...
			Calendar: {
				description: 'Calendar in Weekdays, Weekdays with time scale, or Resources with time scale mode.',
				...
				promptInstruction: 'Use the Softadmin Calendar component as the main component. Choose exactly one mode: "Weekdays", "Weekdays with time scale", or "Resources with time scale". Base shape: { type:"CalendarWeekdays", mode, monthView?, heading?, previousLabel?, nextLabel?, todayLabel?, year?, years?:[string], month?, months?:[string], week?, day?, sidebar?, sidebarHeading?, sidebarDays?:[[weekNumber, day1, day2, day3, day4, day5, day6, day7]], resourceLabel?, resources?:[string], resource?, showResourceDropdown?, filters?:[{ label, control:"dropdown", value?, options?:[string|{label,value}], disabled? }], descriptionToggle?:{label?,checked?,disabled?}, resourceFilterGroups?:[{heading,expanded?,items:[{label,checked?,disabled?}]}], dayHeadings?:[string], dayHeadingsShort?:[string] }. A sidebarDays cell may be a string or {label,key?,marked?,today?,selected?}; use objects when a date range is marked. Sidebar filters use dropdown controls and do not need a generic Filter heading. descriptionToggle is the only feature that uses saToggleWrapper and means show or hide activity descriptions; it appears in the calendar sidebar below its dropdown filters. Checkbox resource filters are for Resources with time scale; use resourceFilterGroups only in that mode instead of a resource dropdown when several resources can be shown simultaneously. Weekdays is a full-month view by default: set monthView:true, use seven dayHeadings from Sunday through Saturday, omit the week picker, and provide five or six complete weeks with seven days each, including boundary dates from adjacent months. The date property must be ISO YYYY-MM-DD; use dateLabel only for boundary labels such as "17 Nov", otherwise the renderer derives the visible date number from date. Weekdays with time scale is week-based and supports multiple weeks; it renders a date row, all-day row, and timed row for each week and also uses timeSlots?:["00:00","03:00",...], slotHeight?, columnWidth?, currentTime?, and timed activities with start:"09:00", end:"10:30". For Resources with time scale use resourceColumns:[{ label, current?, activities:[{ title, description?, start?, end?, allDay?, color?, textColor?, clickable? }] }] plus timeSlots. Saturdays and Sundays normally use redDay:true.',
				renderable: true,
			...
			Grid: {
				...
				promptInstruction: 'Use the Softadmin Grid as the main component. Shape: { type:"ResultGrid", title?, caption?, hitCounter?, grouped?, groupedBy?, groupOptions?:[string], columns:[{ key, label, width?, sorted?, sortDirection?, numeric?, align? }], rowActions?:[{ key, label, icon, iconStyle?, shortLabel? }], rows:[{ [columnKey]: value, disabledActions?:[actionKey] }|{ type:"groupHeader", label, clickable? }|{ type:"subtotal", label?, values:{ [columnKey]: value } }], total?:{ label?, values:{ [columnKey]: value } } }. Place each groupHeader immediately before its data rows and its subtotal immediately after them. The total belongs in total and renders in the grid footer. groupOptions may include hidden grouping fields that are not displayed as table columns. Define row buttons exactly once in component.rowActions. Every ordinary data row always renders the same actions in the same order. A row may only disable shared actions with disabledActions; never put actions on an individual row. Never use saMissingLabel or labels beginning with "Missing label:"; provide realistic user-facing column labels.',
				renderable: true,
				specType: 'ResultGrid',
			...
			NewEdit: {
				...
				promptInstruction: 'Use the Softadmin NewEdit component as the main component. For a simple form use { type:"NewEdit", labels?:"above|before", toc?:boolean|{ heading?, open? }, sections:[{ id?, heading?, width?, fields:[...] }], buttons?:[{ label, variant:"primary|secondary" }] }. To place subgroups next to each other under one shared section heading use sections:[{ heading, subgroups:[{ id?, heading?, width?, fields:[...] }, { ... }] }]. Each subgroup becomes a saFieldsColumn in the same saFieldsRow; omit subgroup headings when only the shared parent heading should be visible. For more complex multi-row forms use rows:[{ id?, heading?, columns:[{ sections:[{ id?, heading?, width?, fields:[...] }] }] }]. Use labels:"before" for dense administration forms and toc:true for long forms with several visible headings. Keep ordinary forms labels above. Fields use only supported control names from the control catalog; group adjacent compact fields with { layout:"siblings", fields:[...] }.',
				renderable: true,
				specType: 'NewEdit',
				specTypes: ['NewEdit']
			},
