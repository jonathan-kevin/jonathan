	...
	const catalog = {
		components: {
			...
			Calendar: {
				...
				promptInstruction: 'Use the Softadmin Calendar component as the main component. Choose exactly one mode: "Weekdays", "Weekdays with time scale", or "Resources with time scale". Base shape: { type:"CalendarWeekdays", mode, heading?, previousLabel?, nextLabel?, todayLabel?, year?, years?:[string], month?, months?:[string], week?, day?, sidebar?, sidebarHeading?, sidebarDays?:[[weekNumber, day1, day2, day3, day4, day5, day6, day7]], resourceLabel?, resources?:[string], resource?, filters?:[{ label, description?, checked?, disabled? }], dayHeadings?:[string], dayHeadingsShort?:[string] }. For Weekdays use weeks:[{ number, id?, label?, days:[{ day, weekday?, date, dateHeading?, today?, current?, redDay?, clickable?, activities?:[{ title, description?, time?, color?, allDay?, clickable? }] }] }]. The date property must be ISO YYYY-MM-DD; day may contain a number or display label, but the renderer derives the visible date number from date. For Weekdays with time scale, also use timeSlots?:["08:00","08:30",...], slotHeight?, columnWidth?, currentTime?, and timed activities with start:"09:00", end:"10:30". For Resources with time scale use resourceColumns:[{ label, current?, activities:[{ title, description?, start, end, color?, textColor?, clickable? }] }] plus timeSlots. Use several weeks only for a dense Weekdays view. Saturdays and Sundays normally use redDay:true.',
				renderable: true,
