...
(async () => {
	...
	try {
		...
		await page.evaluate(() => {
			...
			project.state.headerHtml = project.state.headerHtml.replaceAll('My customer page', 'Stale header');
			project.state.sidebarHtml = project.state.sidebarHtml.replaceAll('Resource bookings', 'Stale menu');
