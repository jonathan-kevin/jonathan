...
The composer uses the shared `saChatContext`/`saPill` styling for real context state. The current page is included by default as a linked Page pill (the main reference), can be removed independently, and stays removed across closing/reopening until restored with Add context. Page context alone does not enable Send. Selected excerpts remain separate source-linked quotes. The plus button opens a keyboard-accessible menu to include the current page, add a context note, or return to select text on the page. Notes can be edited by clicking their pill, removed individually, and sent without a typed message. Sending snapshots notes into the sent message and clears only those draft notes, keeping page context and any next draft independent. Duplicate note text is not added twice. The textarea spacing tracks wrapped pill height; pill removal is available on keyboard focus and touch. All replies remain simulated; no files are uploaded and no AI receives page data.
...
`inline-chat.js` defines a light-DOM `<inline-chat>` component. Shared chat styles and their nested `.saChatInline &` variants live together in `Chat.less`. `InlineChat.less` contains the panel layout, resize handle, and panel-only content. Same-element modifiers use the `.modifier&` form; ancestor contexts use `.ancestor &`.
