...
With `chat-target` pointing to an `<inline-chat>` panel, **Send to chat** instead attaches one removable passage above the composer and focuses the message box; it does not post immediately. A new selection replaces the pending passage. The button shows **Added to chat**. Only Send/Enter posts the draft and passage, followed by a local thinking spinner and word-streamed demo reply. See `inline-chat.md` for details. The feedback-only fallback below applies when no panel is configured.
